package ru.netology.yandexmaps.db

import android.content.Context
import androidx.room.Database
import androidx.room.Room
import androidx.room.RoomDatabase
import ru.netology.yandexmaps.dao.PlaceMarkDao
import ru.netology.yandexmaps.entity.PlaceMarkEntity

@Database(entities = [PlaceMarkEntity::class], version = 1, exportSchema = false)
abstract class PlaceMarkDb: RoomDatabase() {
    abstract fun placeMarkDao(): PlaceMarkDao

    companion object {
        @Volatile
        private var instance: PlaceMarkDb? = null

        fun getInstance(context: Context): PlaceMarkDb {
            return instance ?: synchronized(this) {
                instance ?: buildDatabase(context).also { instance = it }
            }
        }

        private fun buildDatabase(context: Context) =
            Room.databaseBuilder(context, PlaceMarkDb::class.java, "placemarks.db")
                .fallbackToDestructiveMigration()
                .build()
    }
}